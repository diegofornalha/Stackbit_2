...
function HeaderVariantA(props) {
    const { primaryLinks = [], socialLinks = [], actionLinks = [], ...logoProps } = props;
    return (
...
function HeaderVariantB(props) {
    const { primaryLinks = [], socialLinks = [], actionLinks = [], styles = {}, hasTopMargin, ...logoProps } = props;
    return (
        <div className="flex items-stretch relative">
            <SiteLogoLink {...logoProps} />
            {primaryLinks.length > 0 && (
                <ul className="hidden lg:flex border-l border-current divide-x divide-current ml-auto" data-sb-field-path=".primaryLinks">
                    <ListOfLinks links={primaryLinks} inMobileMenu={false} />                
                    <Actions actions={actions} styles={styles.actions} hasTopMargin={!!(title || subtitle || text)} />
